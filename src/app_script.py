...
def main(update, tools, iac, keyword, output):
    # 0. Setup argument parser
    if tools and not update:
        raise IOError("--names can only be used when --update is enabled.")
    ...
    up_tool_list = get_update_tool_list(is_valid, tools, full_tool_list)
    ...
    print(f"Selected user output file format: {output}")
    for out in output:
