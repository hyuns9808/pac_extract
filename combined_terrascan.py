...
# PATTERNS
# Regular expression to find content within JSON file
...
def process_policies(policyData, serviceSpecific):
    # policy = dictionary of given policy pattern
    policy = {}
    for index, element in policyData:
        if(len(element) == 1):
            policy[index] = element
        else:
            policy[index] = element[1:-1].strip()
    policy['serviceSpecific'] = serviceSpecific
    return policy

def append_policies(policySet, policyList):
    # Get only useful information and create tuple to save runtime
    policyList.append({
        'reference_id':     policySet['reference_id'],
        'id':               policySet['id'],
        'category':         policySet['category'],
        'file_loc':         "~\\" + policySet['resource_type'] + '\\' + policySet['file'],
        'severity':         korSeverity(policySet['severity']),
        'description':      policySet['description'],
        'serviceSpecific':  policySet['serviceSpecific']
    })
...
def get_policy_from_folder(serviceName, resultList):
    ...
    for fileName in fileNames:
        if ".json" in fileName:
            with open(f'{policyFolder}\\{serviceName}\\{fileName}') as f: policyContent = f.read()
            policy = process_policies(policy_pattern.findall(policyContent), "O")
            append_policies(policy, resultList)
            count += 1
...
def get_matching_policy_from_folder(serviceName, targetString, resultList):
    ...
    for fileName in fileNames:
        if(".json" in fileName):
            with open(f'{policyFolder}\\{serviceName}\\{fileName}') as f: policyContent = f.read()
            policy = process_policies(policy_pattern.findall(policyContent), "X")
            pattern = rf'\b{re.escape(targetString)}\b'
            match = re.search(pattern, policy['description'], re.IGNORECASE)
            if match:
                print(f"\tMatching policy found! - '{policyFolder}\\{serviceName}'")
                append_policies(policy, resultList)
                count += 1
    if count != 0:
...
def write_policies_to_worksheet(givenWorksheet, policyData):
    ...
    for policy in policyData:
        givenWorksheet.write(row, 0, policy['reference_id'])
        givenWorksheet.write(row, 1, policy['id'])
        givenWorksheet.write(row, 2, policy['category'])
        givenWorksheet.write(row, 3, policy['file_loc'])
        givenWorksheet.write(row, 4, policy['serviceSpecific'])
        givenWorksheet.write(row, 5, policy['severity'])
        givenWorksheet.write(row, 6, policy['description'])
        row += 1
...
def write_to_excel(policyList, toolName, targetString, path):
    ...
    try:
        ...
        headers = ['JSON ID', 'Rego ID', 'Category', 'File Location', targetString + ' Specified', 'Severity', 'Description']
...
policy_path_string = "pkg\\policies\\opa\\rego\\aws"
...
# Get list of folders from Terrascan GitHub repo
